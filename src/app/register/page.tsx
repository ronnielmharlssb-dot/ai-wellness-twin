  ...
  return (
    <main className="min-h-screen bg-[#F7F8FA] dark:bg-[#20201e] transition-colors duration-300">
      <div className="mx-auto flex min-h-screen max-w-md flex-col justify-center px-6 py-12">
        ...
        {/* CASE 3: Direct Public Visit (No Token Provided) */}
        {!invitation && !tokenError && (
          <div className="rounded-3xl border border-slate-200 bg-white p-7 shadow-sm dark:border-[#383734] dark:bg-[#2d2b26] space-y-5 animate-in fade-in duration-200">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500 dark:text-[#a6a6a6]">
                  One single-use invite per employee account.
                </p>
              </div>
            </div>

            <p className="text-xs leading-5 text-slate-600 dark:text-[#a6a6a6]">
              AI Wellness Twin operates on a closed, confidential corporate tenant model. To create an account, click the unique invitation link sent to your work email by your HR administrator.
            </p>

            <div className="pt-2 flex flex-col gap-2">
              <Link href="/register-company">
                <Button className="w-full text-xs font-bold bg-[#60cdff] text-black hover:bg-[#4cc2ff]">
            ...
            {/* Interactive Demo Testing Strip (Allows evaluators to test 1-click single-use invites) */}
            <div className="mt-4 border-t border-slate-100 pt-4 dark:border-[#383734] space-y-2.5">
              <div className="flex items-center gap-1.5 text-xs font-bold text-slate-700 dark:text-[#cfcfce]">
                <Sparkles className="h-3.5 w-3.5 text-amber-500" />
                <span>Test Demo Invitations (Sandbox)</span>
              </div>
              <p className="text-[11px] text-slate-400 dark:text-[#888884]">
                Click an active single-use demo invite below to test the redemption flow:
              </p>

              <div className="space-y-1.5">
                {activeDemoInvites.map((inv) => (
                  <Link
                    key={inv.id}
                    href={`/register?token=${inv.id}`}
                    className="flex items-center justify-between p-2.5 rounded-xl border border-slate-100 bg-slate-50 hover:bg-slate-100 transition text-xs dark:border-[#383734] dark:bg-[#35342e] dark:hover:bg-white/[0.08]"
                  >
                    <div>
                      <p className="font-semibold text-slate-900 dark:text-white truncate">
                        {inv.fullName} ({inv.email})
                      </p>
                      <p className="text-[10px] text-slate-400 dark:text-[#888884]">
